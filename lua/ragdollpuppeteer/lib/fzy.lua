...
---Typed and optimized for GMod Lua
---@source https://github.com/swarn/fzy-lua/blob/e457e9b39292f4cb052735c706673da573209a0e/src/fzy_lua.lua
...
function fzy.has_match(needle, haystack, case_sensitive)
	...
	---@type integer?
	local j = 1
	for i = 1, len(needle) do
		j = find(haystack, sub(needle, i, i), j, true)
		if not j then
			return false
		else
			j = j + 1
...
local function precompute_bonus(haystack)
	...
	for i = 1, len(haystack) do
		local this_char = sub(haystack, i, i)
...
local function compute(needle, haystack, D, M, case_sensitive)
	...
	for i = 1, m do
		haystack_chars[i] = sub(haystack, i, i)
	end
	...
	for i = 1, n do
		...
		local needle_char = sub(needle, i, i)
...
---@return number score higher scores indicate better matches. See also `get_score_min` and `get_score_max`.
...
---@param haystack string
---@param case_sensitive boolean? defaults to false
---@return integer[] indices where `indices[n]` is the location of the `n`th character of `needle` in `haystack`.
---@return number score the same matching score returned by `score`
...
---@param case_sensitive boolean? defaults to `false`
---@return {[1]: integer, [2]: integer[], [3]: number}[] result `{{idx, positions, score}, ...}`
--- an array with one entry per matching line
